...
description: |
  SDD（Spec-Driven Development）设计与执行工作流。负责生成 spec-plan.md / tasks.md（按任务分级）、模型自评、驱动代码执行。
  当用户需要以下场景时触发：
...
> **产物规格以 spec-driven-dev 的任务分级（S/M/L）为准**：S 档免独立设计文档；M/L 档 spec 与 plan 合并为 `spec-plan.md`（测试策略并入「验证方案」章节）；tasks.md 只留勾选项，不复述步骤细节。下述模板按逻辑内容理解，物理文件按档位组织。
...
2. **预自检（Pre-review）**：在触发正式 code-review 前，以执行者视角做一次自我审查（参见 spec-driven-dev 阶段 5）。重点检查：
