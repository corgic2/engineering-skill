...
description: |
  项目工程化需求开发工作流。为任意语言项目提供端到端需求开发能力：代码侧地图（project_wiki）、需求侧语义翻译、红线约束、脚本兜底验证、跨会话记忆（TECH_SPEC/subtasks/timeline）、工程经验库（experience/，提炼工序委托 team-experience-curator）。
...
| 沉淀归档 | git diff + 时间线 | 更新 `TECH_SPEC.md` + 经验条目（可选） | §7/§8/§9 同步，经验提炼检查完成 |
...
设计产物（spec/plan/tasks）交付前逐条自检，违反任一条视为设计未完成。清单从真实项目评审复盘中泛化而来，**项目可按领域增删**；以下为通用基线：
...
1. 读取 `assets/` 下的模板。
2. 在项目根目录生成 `AIRunWorkDocs/`，包含：`docs/`、`project_wiki/`、`red_lines/`、`tools/`、`runtime/`、`agents/`、`experience/`（含 `experience/README.md` 索引模板，条目由沉淀归档阶段按需填充）。
